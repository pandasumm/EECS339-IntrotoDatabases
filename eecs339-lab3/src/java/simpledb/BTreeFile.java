...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if(pid.pgcateg()==BTreePageId.LEAF){
			BTreeLeafPage leafpg= (BTreeLeafPage) Database.getBufferPool().getPage(tid, pid, perm);
			return leafpg;
		}
		BTreeInternalPage internalpg = (BTreeInternalPage) Database.getBufferPool().getPage(tid, pid, Permissions.READ_ONLY);
        Iterator<BTreeEntry> entryiter = internalpg.iterator();
        ...
        while(entryiter.hasNext()){
        	...
        	if(f==null||f.compare(Op.LESS_THAN_OR_EQ, entryfield)){
        		BTreePageId leftchildid = nextentry.getLeftChild();
        		return findLeafPage(tid,leftchildid,perm,f);
        	}
        }
        if(nextentry!=null){
        	BTreePageId rightchildid = nextentry.getRightChild();
        	return findLeafPage(tid,rightchildid,perm,f);
        }
		return null;
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage newpg = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);

		BTreeLeafPage oldrigntsibling = null;
		if (page.getRightSiblingId() != null) {
			oldrigntsibling = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(),Permissions.READ_WRITE);
		}

		int numoftuples = page.getNumTuples();

		Iterator<Tuple> tupleiter = page.reverseIterator();//using reverseIterator() to get right half of leaf page
		for (int i = 0; 2 * i < numoftuples; i++) {
			Tuple t = tupleiter.next();
			page.deleteTuple(t);
			newpg.insertTuple(t);
		}

		newpg.setLeftSiblingId(page.getId());
		newpg.setRightSiblingId(page.getRightSiblingId());
		page.setRightSiblingId(newpg.getId());
		
		if (oldrigntsibling != null) {
			oldrigntsibling.setLeftSiblingId(newpg.getId());
		}
		

		Field parentfield = newpg.iterator().next().getField(keyField);

		BTreeInternalPage parentpg = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), parentfield);

		page.setParentId(parentpg.getId());
		newpg.setParentId(parentpg.getId());

		parentpg.insertEntry(new BTreeEntry(parentfield, page.getId(), newpg.getId()));

		dirtypages.put(parentpg.getId(), parentpg);
		dirtypages.put(page.getId(), page);
		dirtypages.put(newpg.getId(), newpg);

		if (field.compare(Predicate.Op.LESS_THAN, parentfield)) {
			return page;
		} else {
			return newpg;
		} 
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> entryiter = page.reverseIterator();//using reverseIterator() to get right half of internal page
		for (int i = 0; i < numofentries / 2; i++) {
			BTreeEntry e = entryiter.next();
		...
		BTreeInternalPage parentpg = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), midentry.getKey());

		midentry.setLeftChild(page.getId());
		midentry.setRightChild(newpg.getId());
		parentpg.insertEntry(new BTreeEntry(midentry.getKey(), page.getId(), newpg.getId()));
		...
		if (field.compare(Predicate.Op.LESS_THAN, midentry.getKey())) {
			return page;
		} else {
			return newpg;
		}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int numoftuplespg = page.getNumTuples();
		int numoftuplessibling = sibling.getNumTuples();
		int numoftuples = numoftuplespg + numoftuplessibling;
		...
		while (numoftuplespg < numoftuples / 2) {
			Tuple t = tupleiter.next();
			sibling.deleteTuple(t);
			page.insertTuple(t);
			numoftuplespg = numoftuplespg +1;
			numoftuplessibling =  numoftuplessibling - 1;
		}
		entry.setKey(pg.iterator().next().getField(pg.keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> entryiter = leftSibling.reverseIterator();//read the entry from the right-hand-side of the left sibling page
		int numofentries = page.getNumEntries() + leftSibling.getNumEntries();
		while (page.getNumEntries() < numofentries / 2) {
			Field parentfield = leftEntry.getKey();
			BTreeEntry entry = entryiter.next();
			leftEntry.setKey(entry.getKey());
			parent.updateEntry(leftEntry);
			leftSibling.deleteKeyAndRightChild(entry);
			entry.setLeftChild(entry.getRightChild());
			entry.setRightChild(page.iterator().next().getLeftChild());
			entry.setKey(parentfield);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		Iterator<BTreeEntry> entryiter = rightSibling.iterator();//read the entry from the left-hand-side of the right sibling page
		int numofentries = page.getNumEntries() + rightSibling.getNumEntries();
		while (page.getNumEntries() < numofentries/2) {
			Field parentfield = rightEntry.getKey();
			BTreeEntry entry = entryiter.next();
			...
			entry.setLeftChild(page.reverseIterator().next().getRightChild());
			entry.setKey(parentfield);
			page.insertEntry(entry);
		}
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if (rightPage.getRightSiblingId() != null) {
			oldrightsibling = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
