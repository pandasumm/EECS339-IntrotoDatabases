...
public class TupleDesc implements Serializable {
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        // some code goes here
    	for(int i = 0; i < this.TDList.size(); i++) {
    		if(this.TDList.get(i).fieldName == null) {
    			continue;
    		}
    		if(this.TDList.get(i).fieldName.equals(name)) {
    			return i;
    		}
    	}
    ...
    public int getSize() {
    	...
    	int size = 0;
    	for(int i = 0; i < this.TDList.size(); i++) {
    		size += this.TDList.get(i).fieldType.getLen();
    	}
        return size;
    }
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        // some code goes here
    	TupleDesc td3 = new TupleDesc(new Type[0], new String[0]);
    	td3.TDList.addAll(td1.TDList);
    	td3.TDList.addAll(td2.TDList);
        return td3;
    }
    ...
    public String toString() {
    	...
    	String StrDesc = "";
    	for(int i = 0; i < this.numFields(); i++) {
    		StrDesc += this.getFieldType(i).name() + "(" + this.getFieldName(i) + "),";
    	}
    	// for null object
    	if(StrDesc.length()>0) {
